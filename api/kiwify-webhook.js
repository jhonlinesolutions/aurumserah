...
// CONFIRMADO com o payload real de teste em 10/07/2026 — Customer.email e
// Product.product_id batem exatamente. Os fallbacks abaixo continuam como
// segurança extra, mas não deveriam ser necessários.
// ============================================================
...
// Confirmado: a Kiwify manda tanto order_status ("paid") quanto um campo dedicado
// webhook_event_type ("order_approved"). Combinamos os dois sinais — se qualquer um
// bater, consideramos aprovado/reembolsado. Os valores de reembolso/chargeback ainda
// são estimativa (não testamos esses eventos ainda) — confira no log ao testar.
function extractStatusSignals(body) {
  return [
    body?.order_status,
...
export default async function handler(req, res) {
  ...
  const statusSignals = extractStatusSignals(body);
  ...
  // "paid"/"order_approved" confirmados no teste real. Os demais são estimativa —
  // confira no log ao testar "Reembolso" e "Chargeback" e ajuste se necessário.
  const REFUND_STATUSES = [
    'refunded', 'compra_reembolsada', 'refund',
    'order_refunded', 'chargeback', 'chargedback', 'order_chargeback',
  ];
  const APPROVED_STATUSES = ['paid', 'approved', 'compra_aprovada', 'order_approved'];

  if (statusSignals.some((s) => REFUND_STATUSES.includes(s))) {
    const results = await Promise.all(books.map((book) => revokeAccess(book, email)));
    return res.status(200).json({ revoked: results });
  }

  const isApproved = statusSignals.length === 0 || statusSignals.some((s) => APPROVED_STATUSES.includes(s));
  if (!isApproved) {
    console.log('Webhook ignorado — status não tratado:', statusSignals);
    return res.status(200).json({ ignored: true, reason: 'status not handled', statusSignals });
  }
